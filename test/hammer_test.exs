defmodule HammerTest do
  ...
  test "make_rate_checker" do
    check = make_rate_checker("some-prefix:", 10000, 2)
  ...
  test "returns {:ok, 1} tuple on first access" do
    assert {:allow, 1} = check_rate("my-bucket", 10_000, 10)
  end

  test "returns {:ok, 4} tuple on in-limit checks" do
    assert {:allow, 1} = check_rate("my-bucket", 10_000, 10)
    assert {:allow, 2} = check_rate("my-bucket", 10_000, 10)
    assert {:allow, 3} = check_rate("my-bucket", 10_000, 10)
    assert {:allow, 4} = check_rate("my-bucket", 10_000, 10)
  end

  test "returns expected tuples on mix of in-limit and out-of-limit checks" do
    assert {:allow, 1} = check_rate("my-bucket", 10_000, 2)
    assert {:allow, 2} = check_rate("my-bucket", 10_000, 2)
    assert {:deny, 2} = check_rate("my-bucket", 10_000, 2)
    assert {:deny, 2} = check_rate("my-bucket", 10_000, 2)
  end

  test "returns expected tuples on 1000ms bucket check with a sleep in the middle" do
    assert {:allow, 1} = check_rate("my-bucket", 1000, 2)
    assert {:allow, 2} = check_rate("my-bucket", 1000, 2)
    assert {:deny, 2} = check_rate("my-bucket", 1000, 2)
    :timer.sleep 1001
    assert {:allow, 1} = check_rate("my-bucket", 1000, 2)
    assert {:allow, 2} = check_rate("my-bucket", 1000, 2)
    assert {:deny, 2} = check_rate("my-bucket", 1000, 2)
  ...
  test "returns expected tuples on delete_buckets" do
    ...
    assert {:deny, 2} = check_rate("my-bucket1", 1000, 2)
    assert {:allow, 1} = check_rate("my-bucket2", 1000, 2)
    assert {:allow, 2} = check_rate("my-bucket2", 1000, 2)
    assert {:deny, 2} = check_rate("my-bucket2", 1000, 2)
    assert {:ok, 1} = delete_buckets("my-bucket1")
    assert {:allow, 1} = check_rate("my-bucket1", 1000, 2)
    assert {:deny, 2} = check_rate("my-bucket2", 1000, 2)

    assert {:ok, 0} = delete_buckets("unknown-bucket")
  end
